...
public class SurvivalSpawnerScreen extends HandledScreen<SurvivalSpawnerScreenHandler> {
	...
	protected void init() {
		...
		this.spawnDelayTextField.setChangedListener(text -> {
			if(SurvivalSpawnerScreen.this.settings != null && !text.isEmpty()) {
				try {
					int delay = Integer.parseInt(text.trim());
					SurvivalSpawnerScreen.this.settings.setSpawnDelay(delay);
					SurvivalSpawnerScreen.this.sendSettings();
		...
		this.modeButton = this.addButton(new ButtonWidget(40, offset + 50, 160, 20, new LiteralText(SpawnerSettings.Mode.values()[0].text),
				button -> {
					if(SurvivalSpawnerScreen.this.settings != null) {
